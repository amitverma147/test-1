...
function bypassAuthForTesting(req) {
  ...
  console.log('✅ Auth bypass (quick_messages): Allowing access for testing');
  return true;
}

module.exports = async function quickMessagesHandler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      const { data, error } = await supabaseAdmin.from('quick_messages').select('message').limit(200);
      if (error) {
        console.error('Failed to fetch quick_messages', error);
    ...
    console.error('server_handlers/quick_messages error', err);
