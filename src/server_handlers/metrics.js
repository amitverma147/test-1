...
module.exports = async function metricsHandler(req, res) {
  try {
    ...
    if (mode === 'today') {
      ...
      return res.status(200).json({ mode: 'today', totalCreatedToday: total, openCreatedToday: open, completedCreatedToday: completed, revenueToday: revenue, followupsDueToday: followups });
    }
    ...
    console.error('server_handlers/metrics error', err);
