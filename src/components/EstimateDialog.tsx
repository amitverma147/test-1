...
export function EstimateDialog({ open, onOpenChange, job }: EstimateDialogProps) {
  ...
  const handleGenerateBill = (format: 'txt' | 'csv' | 'xlsx' = 'xlsx') => {
    try {
      const summary: any = {
        JobCard: job?.jobCardNumber || '',
        Date: new Date().toLocaleDateString(),
        Model: (job as any).vehicleModel || job?.model || '',
        Registration: (job as any).vehicleRegNo || job?.regNo || '',
        Customer: job?.customerName || '',
        Phone: (job as any).customerPhone || job?.customerMobile || '',
      ...
      const lineItems = selectedParts.map((it) => ({
        Type: 'Part',
        Name: it.part.name,
        Quantity: it.quantity,
        UnitPrice: it.part.price,
        Total: it.part.price * it.quantity,
      })).concat(services.filter(s => s.selected).map(s => ({
        Type: 'Service',
        Name: s.name,
        Quantity: 1,
        UnitPrice: s.price,
      ...
      if (format === 'csv') {
        ...
        a.href = url;
        a.download = `Estimate_${job?.jobCardNumber}_${Date.now()}.csv`;
        ...
        XLSX.utils.book_append_sheet(wb, wsLines, 'LineItems');
        const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
        const blob = new Blob([wbout], { type: 'application/octet-stream' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `Estimate_${job?.jobCardNumber}_${Date.now()}.xlsx`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        ...
        // fallback to txt
        const billText = `Estimate for ${job?.jobCardNumber} - Total: ₹${grandTotal}`;
      ...
      console.error('Estimate export failed', err);
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <Button 
              ...
              onClick={() => handleGenerateBill('xlsx')}
